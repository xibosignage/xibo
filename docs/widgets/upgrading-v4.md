...
This documentation has detailed information for how to [create a module](creating-a-module) using the new developer tools. A key decision is whether your module needs to handle data and how you handle that data.
...
To create properties for v4, we need to convert them from the v3 `moduleName-form-edit.twig` to the new `moduleName.xml` file in the [properties](xml-definition#content-3-property) block. Note that, to be compatible with existing v3 modules, the property IDs must match those required by the `edit` function in `lib/Controller/moduleName.php`.
...
By also using the render flow [methods](creating-a-module#content-methods), it is possible to manipulate the data before rendering even further:
...
We can pass data (variables and methods) between these methods by using the [Xibo Interactive Control](creating-a-module#content-using-the-xibo-interactive-control), if needed.
